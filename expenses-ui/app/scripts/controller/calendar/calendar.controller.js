    ...
    return {
      ...
      link: function (scope, element, attrs, modelCtrl) {
        ...
        scope.reserve = function(date, hour){
          ...
          if (now.isBefore(date)) {
            ...
            modalInstance.result.then(function (event) {
              scope.setDates();
              if (scope.user.role.id != CONSTANTS.ROLES.ADMIN && !scope.user.skipMessage) {
        ...
        scope.cancel = function(event){
          if (scope.user.role.id === CONSTANTS.ROLES.ADMIN || event.user.id === scope.user.id) {
            confirmm.confirm("Eliminar evento " + event.title, function () {
              scope.loading = true;
              AppointmentService.deleteEvent(event.id, scope.user.id).then(function () {
                if (Response.saved.status == 'success') {
                  notiffy.success(Response.saved.message);
